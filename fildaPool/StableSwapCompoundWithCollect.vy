...
# This contract handle Filda token
contract HandleLend:
    def swap_FILDA_to_fToken(isHUSD: bool, _value: uint256) -> uint256: modifying
    def claim_FILDA(): modifying
...
def collect_lend_token():
    HandleLend(self.handle_lend_contract_address).claim_FILDA()
    ...
    minted: uint256 = HandleLend(self.handle_lend_contract_address).swap_FILDA_to_fToken(is_HUSD, lend_token_balance)
    if is_HUSD:
