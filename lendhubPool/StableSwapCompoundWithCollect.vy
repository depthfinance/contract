...
# This contract handle lending platform token.
contract HandleLendPlatformToken:
    def sell_lending_platform_token(isHUSD: bool, _value: uint256) -> uint256: modifying
    def claim_lending_platform_token(): modifying
...
SetHandleLendContractAddress: event({lendAddress: indexed(address)})
RampA: event({
    old_A: uint256,
    new_A: uint256,
...
is_killed: bool
handle_lend_contract_address:public(address)
lend_token: ERC20
@public
def __init__(_coins: address[N_COINS], _underlying_coins: address[N_COINS],
             _pool_token: address,
             _A: uint256, _fee: uint256, _handle_lend_contract_address: address, _lend_contract_address: address):
    """
    ...
    self.handle_lend_contract_address = _handle_lend_contract_address
...
def collect_lend_token():
    HandleLendPlatformToken(self.handle_lend_contract_address).claim_lending_platform_token()
    lend_token_balance: uint256 = self.lend_token.balanceOf(self)
    # Stop if no token claimed.
    ...
    minted: uint256 = HandleLendPlatformToken(self.handle_lend_contract_address).sell_lending_platform_token(is_HUSD, lend_token_balance)
    if is_HUSD:
