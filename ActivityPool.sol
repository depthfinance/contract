...
contract ActivityPool is Ownable,Pausable {
    ...
    function transferReward(uint256 _reward) internal{
    ...
    function unLock(uint256 _amount) external whenNotPaused{
        ...
        if (_amount>0){
            IERC20(lockToken).safeTransfer(msg.sender,_amount);
            _user.lockAmount = _user.lockAmount.sub(_amount);
            totalLockAmount = totalLockAmount.sub(_amount);
        }
        _user.rewardDebt = _user.lockAmount.mul(rewardPerLock).div(10**12);
        emit Lock(msg.sender,_amount);
    }
